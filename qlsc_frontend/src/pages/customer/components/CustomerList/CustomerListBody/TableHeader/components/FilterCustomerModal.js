...
import '../styles/filterCustomerModal.scss';
...
function FilterOrderCollationsModal(props) {
  ...
  const closeDropdownSource = (e) => {
    ...
    if (
      ...
      || (!e.target.className.includes('dropdown-item-customer-filter')
  ...
  const showFilterOption = (list_option, select) => {
    ...
    return (
      <div className="filter-customer-option">
        <button
          type="button"
          className="dropdown-toggle customer-dropdown-button"
          data-toggle="dropdown"
        >
          {getOptionName(select)}
          <Icons.Arrow />
        </button>
        <div className="dropdown-menu customer-dropdown-menu">{elmFO}</div>
      </div>
    );
  };
  ...
  const renderShowOption = (option) => {
    ...
    //     return (
    //       <div
    //         className="dropdown-item dropdown-item-customer-filter item-store"
    //         key={index}
    ...
    //           </span>
    //         </div>
    //         <div className="dropdown-item-customer-filter">
    //           {store.label}
    //         </div>
    //       </div>
    //     );
    //   });
    //   return (
    //     <div className="filter-customer-detail">
    //       <button
    //         type="button"
    //         className="dropdown-toggle button-fillter-customer-dropdown"
    ...
    //       </button>
    //       <div
    //         className={`dropdown-menu dropdown-item-customer-filter ${
    //           showDropdownBranch ? 'show' : null
    //         }`}
    //         id="dropdown-customer-source"
    //       >
    //         {listStoreElm}
    //       </div>
    //     </div>
    ...
    if (option === 'statusWork') {
      const list_button_work = default_status_work.map((a, index) => {
        return (
          <button
            key={index}
            className={`filter-customer-button ${status === index + 1 ? 'active' : null}`}
  ...
  const showFilterRow = (option) => {
    ...
    return (
      <div className="filter-customer-line" style={{ display: 'flex' }}>
        {showFilterOption(options, option)}
        {renderShowOption(option)}
        <div
          className="filter-customer-bin"
          onClick={() => onSelectOption(option)}
        >
          <Icons.Bin />
        </div>
  ...
  return (
    <div id="filter-customer-modal-wrapper" ref={myRef}>
      <div id="filter-customer-modal-header">Hiển thị đơn hàng theo</div>
      <div id="filter-customer-modal-body">
        {showFilterWrapper()}
        {last_filter_option.length !== 1 ? (
          <div className="filter-customer-line">
            {showFilterOption(last_filter_option, 'select')}
          </div>
        ) : null}
      </div>
      <div id="filter-customer-modal-footer">
        <button
          type="button"
          id="filter-customer-modal-action"
          onClick={() => filter()}
        >
            Lọc
        </button>
